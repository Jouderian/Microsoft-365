...
- **[x]** 2026-07-04: [Feature] Criado o script `listarCredenciaisEntraId.ps1` sob a pasta `entraId/` para auditoria e listagem de credenciais (Secrets e Certificados) das aplicações e service principals do Entra ID.
- **[x]** 2026-09-16: [Padronização] Definidas duas zonas de nomenclatura em `.agents/rules/general.md` (`camelCase` pt-BR para código do projeto, `kebab-case` inglês para `.agents/`), com renomeação de `code-standards.md`, `new-script.md`, `update-repository.md`, `project-vision/`, `system-architecture/` e `README.md`.
- **[x]** 2026-09-16: [Configuração] Criado `CLAUDE.md` importando as regras de `.agents/rules/`, mais as regras `response-style.md` (estilo de resposta no chat) e `backlog-maintenance.md` (baixa automática de pendências em `todoDone.md`).
- **[x]** 2026-09-16: [Padronização] Unificado o frontmatter YAML das 6 regras de `.agents/rules/` em um único bloco com `trigger` e `description`, e preenchida a `description` vazia de `wiki-maintenance.md`.
- **[x]** 2026-09-16: [Correção] Corrigida a função `removeQuebraDeLinha` em `bibliotecaDeFuncoes.ps1`, que só removia CRLF: as substituições de `'`n'` e `'`r'` usavam aspas simples e trocavam o literal crase+letra, sem efeito sobre quebras reais. Substituídas por `-replace '\r?\n|\r'`, cobrindo CRLF, LF e CR isolado.
