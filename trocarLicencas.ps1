...
if ($Users.Count -eq 0){
  Write-Host "Nenhum usuario encontrado no arquivo $arquivo" -ForegroundColor Red
  Exit
}
...
$Users | ForEach-Object {
  try {
    ...
    if ($params.Count -gt 1){  # Mais que apenas UserId
    ...
    Write-Host "Problema com a troca da licenca na caixa $($_.eMail): $($_.Exception.Message)" -ForegroundColor Red
  }
}
...
Write-Host "$($final.ToString('dd/MM/yy HH:mm:ss')) - Tempo de duracao: $((NEW-TIMESPAN -Start $inicio -End $final).ToString())"