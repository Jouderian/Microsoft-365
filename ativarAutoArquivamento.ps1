...
$caixasPostais = Get-Mailbox -Filter "ArchiveStatus -eq 'None' -and RecipientTypeDetails -eq 'UserMailbox'"
...
Foreach ($caixaPostal in $caixasPostais){
  ...
  $usuario = Get-MsolUser -ObjectId $caixaPostal.ExternalDirectoryObjectId
